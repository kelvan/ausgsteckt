...
class IncompleteBuschenschankList(PageTitleMixin, ListView):
    ...
    def get_queryset(self):
        ...
        addr_exclude = Q(
            tags__has_keys=["addr:housenumber", "addr:city", "addr:country", "addr:postcode"],
            tags__has_any_keys=["addr:street", "addr:place"],
        )
        contact_exclude = (
            Q(tags__has_any_keys=["website", "contact:website"])
            and Q(tags__has_any_keys=["contact:phone", "phone"])
            and Q(
...
class BrokenURLView(PageTitleMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        report_path = os.path.join(settings.MEDIA_ROOT, "broken_websites_report.csv")
